...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log('✅ PDF profesional generado exitosamente:', pdfSignedUrl);
...
async function generatePDFFromData(cotizacionData) {
  ...
  try {
    ...
    // Datos reales de FullColor.ec
    const companyInfo = {
      name: 'FullColor.ec',
      tagline: 'Soluciones gráficas profesionales',
      address: 'Rocafuerte 302 y 23 Abril, Machala, Ecuador',
      phone: '+593 98 870 5311',
    ...
    // ============================================
    // Logo posicionado más arriba con margen profesional (10mm desde el borde)
    const headerTop = 10;
    ...
    // Información de la derecha (Cotización #, Date, Valid Until) SIN TARJETA
    ...
    doc.setFontSize(8);
    doc.setTextColor(colorTexto[0], colorTexto[1], colorTexto[2]);
    const termsText = doc.splitTextToSize('Pago dentro de 30 días. Entrega entre 2-3 semanas después de la confirmación del pago. Todos los artículos sujetos a disponibilidad.', notesWidth);
    ...
    doc.setFontSize(9);
    doc.setTextColor(colorNegro[0], colorNegro[1], colorNegro[2]);
    doc.text('¡Gracias por su preferencia!', pageWidth / 2, footerY + 7, {
      align: 'center'
    });
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(7.5);
    doc.setTextColor(colorMuted[0], colorMuted[1], colorMuted[2]);
    doc.text(`${companyInfo.email}  •  ${companyInfo.phone}`, pageWidth / 2, footerY + 12, {
      align: 'center'
    });
